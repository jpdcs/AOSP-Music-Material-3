...
public class MediaPickerActivity extends BaseActivity implements MusicUtils.Defs
{
    ...
    public void init() {
        ...
        mMediaList = findViewById(R.id.list);
        mMediaList.setLayoutManager(new LinearLayoutManager(this));
    ...
    private void MakeCursor() {
        ...
        if (type.equals("video/*")) {
            ...
            if (mFirstYear == null && (intent.getType() != null && intent.getType().equals("media/*"))) {
                // video has no year column
                c = MusicUtils.query(this, MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
                    videocols, null , null, mSortOrder);
                if (c != null) {
    ...
    static class PickListAdapter extends CursorRecyclerViewAdapter<PickListAdapter.ViewHolder> {
        int mTitleIdx;
        int mArtistIdx;
        int mAlbumIdx;
        int mMimeIdx;
        MediaPickerActivity mActivity;
        ...
        PickListAdapter(MediaPickerActivity activity, int layout, Cursor cursor, String[] from, int[] to) {
        ...
        public void onBindViewHolder(ViewHolder vh, Cursor cursor) {
            ...
            if (name == null || name.equals(MediaStore.UNKNOWN_STRING)) {
                builder.append(vh.itemView.getContext().getString(R.string.unknown_album_name));
            } else {
                builder.append(name);
            }
            builder.append("\n");
            ...
            if (name == null || name.equals(MediaStore.UNKNOWN_STRING)) {
                builder.append(vh.itemView.getContext().getString(R.string.unknown_artist_name));
            } else {
                builder.append(name);
            }
            ...
            final long id = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media._ID));
