    ...
    implements MusicUtils.Defs, ServiceConnection
{
    ...
    private void showSortMenu() {
        ...
        PopupMenu popup = new PopupMenu(this, toolbar != null ? toolbar : mAlbumList);
        popup.getMenuInflater().inflate(R.menu.sort_options, popup.getMenu());

        // Date added is not directly available in Albums table
        ...
        popup.setOnMenuItemClickListener(item -> {
            ...
            if (itemId == R.id.sort_name_asc) {
                ...
                sortOrder = MediaStore.Audio.Albums.ARTIST + " DESC";
            }

            MusicUtils.setStringPref(this, "album_sort_order", sortOrder);
            getAlbumCursor(mAdapter.getQueryHandler(), null);
            return true;
        });
        popup.show();
    ...
    private Cursor getAlbumCursor(AsyncQueryHandler async, String filter) {
        ...
        String sortOrder = MusicUtils.getStringPref(this, "album_sort_order", MediaStore.Audio.Albums.DEFAULT_SORT_ORDER);
    ...
    static class AlbumListAdapter extends CursorRecyclerViewAdapter<AlbumListAdapter.ViewHolder> implements SectionIndexer {
        ...
        AlbumListAdapter(Context context, AlbumBrowserActivity currentactivity,
            ...
            mDefaultAlbumIcon = (BitmapDrawable) MusicUtils.getBitmapDrawable(context, R.drawable.albumart_mp_unknown_list);
        ...
        public void onBindViewHolder(ViewHolder vh, Cursor cursor) {
            ...
            final ImageView iv = vh.icon;
            long aid = cursor.getLong(0);
            vh.albumId = aid;
            if (unknown) {
                iv.setImageDrawable(null);
            } else {
                Drawable d = MusicUtils.getArtworkFromCache(aid);
                if (d != null) {
                    iv.setImageDrawable(d);
                } else {
                    iv.setImageDrawable(mDefaultAlbumIcon);
                    mExecutor.execute(() -> {
                        final Drawable artwork = MusicUtils.getCachedArtwork(vh.itemView.getContext(), aid, mDefaultAlbumIcon);
                        if (artwork != null) {
                            vh.itemView.post(() -> {
                                if (vh.albumId == aid) {
                                    iv.setImageDrawable(artwork);
                                }
                            });
                        }
                    });
            ...
            ImageView indicator = vh.play_indicator;
            if (currentalbumid == aid) {
                indicator.setImageDrawable(mNowPlayingOverlay);
            } else {
